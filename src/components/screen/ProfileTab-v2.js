...
export default class Main extends React.Component {
  static navigationOptions = {
    tabBarIcon: ({ tintColor }) => (
      <Image style={{ width: 30, height: 30, resizeMode: 'center' }} source={require('../assets/home/icon_account_inactive.png')}/>
    )
  }
  ...
  componentWillMount() {
    return fetch('http://www.splashbase.co/api/v1/images/search?query=car').then((response) => response.json()).then((responseJson) => {
        ...
        function() { console.warn(responseJson.images) })
    ...
    return (
      <ScrollView>
        <View style={{ flex: 1, backgroundColor: '#fafafa' }}>
            <View style={{ flex: 1, flexDirection: 'row', paddingTop: 10, paddingBottom: 10, borderBottomColor: '#bbb', borderBottomWidth: StyleSheet.hairlineWidth }}>
                <View style={{ flex: 1, justifyContent: 'center' }}>
                </View>
                <View style={{ flex: 5, justifyContent: 'center' }}>
                  <Text style={{ color: 'black', fontSize: 14, textAlign: 'center' }}>{currentUser && currentUser.email}</Text>
                </View>
                <View style={{ flex: 1, justifyContent: 'center' }}>
                  <TouchableOpacity onPress={this.handleLogout}>
                    <Image style={{ width: 30, height: 30 }} source={require('../assets/home/icon_more.png')}/>
                  </TouchableOpacity>
                </View>
            </View>

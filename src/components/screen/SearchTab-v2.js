        ...
        renderItem={ ({item}) =>
          <View style={{ flexWrap: 'wrap' }}>
            <TouchableOpacity onPress={() => { Alert.alert(item.guid) }}>
              <Image style={{ height: this.state.height, width: this.state.width, margin: 0 }} source = {{ uri: item.picture }} onPress={()=>alert(item.picture)}/>
            </TouchableOpacity>
          </View>
        }
        ...
        renderItem={ ({item}) =>
          <ListView
            ...
            renderRow={(rowData) => 
              <View style={{ flexWrap: 'wrap' }}>
                <TouchableOpacity onPress={() => { Alert.alert(rowData.guid) }}>
                  <Image style={{ height: this.state.height, width: this.state.width, margin: 0 }} source = {{ uri: rowData.picture }} onPress={()=>alert(rowData.picture)}/>
                </TouchableOpacity>
              </View>
            }
          />
